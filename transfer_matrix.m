function T = transfer_matrix(temp, Gamma)
% TRANSFER_MATRIX - Create transfer matrix for every possible dinucleotide step
% transfer_matrix(temp, Gamma) creates a 4x4 cell array of transfer
%    matrices at temperature "temp" and torque "Gamma".
  ...
  T = cell(4,4);
  for s=1:4
    S1diag = p.legw(n1).*p.xi(n1) .* Tmorse(p.xi(n1),p.D(s),p.a(s),p.r0,beta);
    S1 = sparse(n1,n1,S1diag,p.ML,p.ML);
    for t=1:4
      S2 = Tstack(p.xi(i1),p.xi(i2),p.K(s,t),p.alpha(s,t),p.r0,beta);
      S2 = reshape(S2,[p.ML,p.ML]);
      
      S3 = p.chebw.*Ttwist(p.xi(j1),p.xi(j2),p.chebz(j3),p.E(s,t),p.h,...
